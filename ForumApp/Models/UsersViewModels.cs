...
namespace ForumApp
{
    ...
    class UsersView
    {
        ...
        public void Update()
        {
            ...
            {
                koneksi.bukaKoneksi();
                string query = "UPDATE users SET Username = @Username, Email = @Email, Password = @Password, Level = @Level WHERE UserId = @UserId AND Level = 1";
                using (SqlCommand command = new SqlCommand(query, koneksi.con))
                {
                    command.Parameters.AddWithValue("@Username", Username);
                    command.Parameters.AddWithValue("@Email", Email);
        ...
        public void Delete()
        {
            ...
            {
                koneksi.bukaKoneksi();
                string query = "DELETE FROM users WHERE UserId = @UserId AND Level = 1";
                using (SqlCommand command = new SqlCommand(query, koneksi.con))
                {
                    command.Parameters.AddWithValue("@UserId", UserId);
